...
import com.hyphenate.easeui.widget.EaseReactionView;
...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void initView() {
        ...
        if (helper != null && helper.getStyle(context) != null) {
            if (helper.getStyle(context).getItemShowType() == 1) {
                showSenderType = false;
            } else if (helper.getStyle(context).getItemShowType() == 2) {
                showSenderType = true;
            }
        }
        onInflateView();
        ...
        threadRegion = findViewById(R.id.thread_view);
    ...
    private void setClickListener() {
        ...
        if(threadRegion != null) {
            threadRegion.setOnClickListener(v -> {
                EMChatThread info = message.getChatThread();
                if(info != null && !TextUtils.isEmpty(info.getChatThreadId())) {
                    if (itemClickListener != null && itemClickListener.onThreadClick(message.getMsgId(), info.getChatThreadId(),info.getParentId())){
                        return;
                    }
                }else {
            ...
            threadRegion.setOnLongClickListener(v -> {
                ...
                if(info != null && !TextUtils.isEmpty(info.getChatThreadId())) {
                    if (itemClickListener != null) {
                        return itemClickListener.onThreadLongClick(v, message.getMsgId(), info.getChatThreadId(),info.getParentId());
                    }
                }else {
                    EMLog.e(TAG, "message's thread info is null");
                }
