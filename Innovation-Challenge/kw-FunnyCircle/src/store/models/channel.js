 ...
 * @property {boolean} groupNoteVisible: 是否展示群接龙Modal
...
const channel = {
  state: {
    ...
    channelGroupNoteVisible: false,
    channelGroupNote:new Map(),
    inviteVisible: false,
    threadMap: new Map(),
  ...
  reducers: {
    ...
    updateGroupNote(state,payload){
      const { channelGroupNote } = state;
       console.log(channelGroupNote)
       setTimeout(() => {
        console.log(channelGroupNote)
       }, 1000);
      return {
        ...state,
        channelGroupNote: payload  //更新接龙
      };
    },
  ...
  effects: {
    ...
    insertGroupNoteMessage(info){
      this.updateGroupNote(info)
    },
    setChannelGroupNoteVisible(visible) {
      this.updateState({ channelGroupNoteVisible: visible });
    },
