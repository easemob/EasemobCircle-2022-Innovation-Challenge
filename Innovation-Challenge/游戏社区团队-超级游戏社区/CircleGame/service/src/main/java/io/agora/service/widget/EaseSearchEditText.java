...
public class EaseSearchEditText extends AppCompatEditText {
    ...
    public EaseSearchEditText(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public EaseSearchEditText(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        if(attrs != null) {
            ...
            mLeftHeight = ta.getDimension(R.styleable.EaseSearchEditText_search_edit_drawable_left_height, DEFAULT_SIZE);
            mLeftWidth = ta.getDimension(R.styleable.EaseSearchEditText_search_edit_drawable_left_width, DEFAULT_SIZE);
    ...
    private void setDrawable() {
        ...
        if(left == null) {
            left = ContextCompat.getDrawable(mContext, R.drawable.seach_white);
        }
        if(right==null) {
            right=ContextCompat.getDrawable(mContext, R.drawable.search_delete_light);
        }
        if(left != null && (mLeftWidth != 0 && mLeftHeight != 0)) {
            left.setBounds(0, 0, (int)mLeftWidth, (int)mLeftHeight);
        }
        ...
        if(background == null) {
            background = ContextCompat.getDrawable(mContext, R.drawable.shape_common_search_bg);
