...
public class EaseSidebar extends View{
	...
	private static final String DEFAULT_COLOR = "#c4c4c4";
	...
	private void initAttrs(AttributeSet attrs) {
		if(attrs != null) {
			...
			int topTextId = a.getResourceId(R.styleable.EaseSidebar_side_bar_top_text, -1);
			if(topTextId != -1) {
				topText = context.getResources().getString(topTextId);
			}else {
				topText = a.getString(R.styleable.EaseSidebar_side_bar_top_text);
			}
			int textColorId = a.getResourceId(R.styleable.EaseSidebar_side_bar_text_color, -1);
			if(textColorId != -1) {
				mTextColor = ContextCompat.getColor(context, textColorId);
			}else {
				mTextColor = a.getColor(R.styleable.EaseSidebar_side_bar_text_color, Color.parseColor(DEFAULT_COLOR));
			}
			int focusBgColor = a.getResourceId(R.styleable.EaseSidebar_side_bar_focus_bg_color, -1);
			if(focusBgColor != -1) {
				mFocusBgColor = ContextCompat.getColor(context, focusBgColor);
			}else {
				mFocusBgColor = a.getColor(R.styleable.EaseSidebar_side_bar_focus_bg_color, Color.TRANSPARENT);
			}
			delayDisappearTime = a.getInteger(R.styleable.EaseSidebar_side_bar_delay_disappear_time, 500);
			mTextSize = a.getDimension(R.styleable.EaseSidebar_side_bar_text_size, DEFAULT_TEXT_SIZE);
			int bgId = a.getResourceId(R.styleable.EaseSidebar_side_bar_background, -1);
			if(bgId != -1) {
				mBgColor = ContextCompat.getColor(context, textColorId);
			}else {
				mBgColor = a.getColor(R.styleable.EaseSidebar_side_bar_background, Color.TRANSPARENT);
			}
			int headArrays = a.getResourceId(R.styleable.EaseSidebar_side_bar_head_arrays, -1);
			if(headArrays != -1) {
