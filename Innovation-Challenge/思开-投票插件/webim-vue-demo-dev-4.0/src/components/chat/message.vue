<template>
  <div class="messagebox" v-show="toggleWindows">
    ...
    <create-vote-modal ref="createVoteModal"></create-vote-modal>
    <vote-modal ref="voteModal"></vote-modal>
    <!-- fix 移动到全局 -->
...
import CreateVoteModal from "../vote/createVoteModal";
...
export default {
  ...
  methods: {
    ...
    onCreateVoteClick() {
      this.$refs.createVoteModal.$once('createVoteOk', (data) => {
        this.onSendText({
          chatType: this.type,
          chatId: this.$data.activedKey[this.type],
          message: `我发起了 ${data.voteName} 的投票，大家踊跃参与吧！`,
          vote: {voteName: data.voteName, voteId: data.voteId}
        });
      })
      this.$refs.createVoteModal.changeModal();
    },
    onMessageClick(message) {
      if (message.vote) {
        this.$refs.voteModal.changeModal(message.vote);
      }
    },
    setReportMsgId(id) {
          ...
          success: () => {
            ...
            if (this.copyMsgObj[key.id].flag) {
              ...
              me.getHistoryMessage({
                name: key.id, isGroup: true, success: () => {
                  this.onGetCurrentChatObjMsg({type: this.type, id: key.id});
                }
              });
            }
