...
class ChatViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.tableView.register(MessageGroupNoteCell.self, forCellReuseIdentifier: "groupNote")
    ...
    private func setupChatInputViewHandle() {
        ...
        self.chatInputView.didSelectedMoreItemHandle = { [unowned self] item in
            ...
            case .groupNote:
                let vc = GroupNoteViewController(model: GroupNote.createdGroupNote(), creating: true)
                vc.didSendHandle = { [unowned self] model in
                    
                    if let data = try? JSONEncoder().encode(model),
                        let s = String(data: data, encoding: .utf8) {
                        
                        let messageBody = EMCustomMessageBody(event: GroupNoteEvent,
                                            customExt: ["body": s])
...
extension ChatViewController: UITableViewDataSource {
    ...
    private func dequeueReusableCell(message: EMChatMessage, indexPath: IndexPath) -> MessageBaseCell? {
        if message.body.type == .text {
            ...
            return circleCell
        } else if message.body.type == .custom,
                    let customBody = message.body as? EMCustomMessageBody,
                    customBody.event == GroupNoteEvent {
            let cell = tableView.dequeueReusableCell(withIdentifier: "groupNote") as? MessageBaseCell
            if let groupNoteCell = cell as? MessageGroupNoteCell {
                groupNoteCell.didTapBlock = { [unowned self] groupNote in
                    let vc = GroupNoteViewController(model: groupNote, creating: false)
                    vc.didSendHandle = { [unowned self] model in
