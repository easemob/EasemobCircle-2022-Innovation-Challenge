...
import QrcodeIcuSdk from 'qrcodeicu-sdk'
...
function onGetPushConfig(message) {
  ...
  WebIM.conn.getSilentModeForConversation(option).then(res => {
    ...
    Vue.$store.commit('updatePushConfig', {user: option.conversationId, type: dataLength ? 'add' : 'remove'})
  })
}
...
WebIM.conn.listen({
  ...
  onTextMessage: function (message) {
    ...
    const robotPhone = 13550305634;
    if (from != robotPhone && WebIM.conn.user == robotPhone) {
      const content = message.data;
      if (content === "每日海报") {
        let option = {
          type: "txt",
          msg: `收到生成海报的指令，请稍等几秒钟...`,
          priority: "high",
          to,
          chatType: type,
        };
        ...
        }).render("ed9def42-b724-4386-907b-6041e56638e3", "5b90dc39-e979-4a15-8c5a-dcd6c0e698c7")
          .addTextParam("手机号", from)
          .fetch()
          .then((data) => {
            let option = {
              type: "txt",
              msg: ` ${from}， 你的海报 ：${data.url}`,
              priority: "high",
              to,
              chatType: type,
            };
    ...
    if (message.ext && message.ext.action === 'invite') {
      ...
      if (callVideo.callStatus > CALLSTATUS.idle) { // 正忙
        if (message.ext.callId == callVideo.callId) { // 多人会议中邀请别人
          ...
          return Vue.$store.dispatch('answerCall', {
            result: 'busy',
