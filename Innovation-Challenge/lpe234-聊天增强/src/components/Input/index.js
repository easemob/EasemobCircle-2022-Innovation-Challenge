...
const Input = (props) => {
  ...
  const doScreenshot = () => {
    console.log("尝试进行截图");
    new ScreenShot({enableWebRtc: false, completeCallback: callback});
  };

  // 截图确认按钮回调
  const callback = (base64Img) =>{
    let resFile = dataURLToFile(base64Img, ".jpg");
    const getImgMsg = (target) => {
      return createMsg({
        chatType: chatType,
        type: "img",
    ...
    <Menu
      items={[
        ...
        },
        {
          key: "screenshot",
          label: (
              <div className="circleDropItem" onClick={() => doScreenshot()}>
                <Icon name="img" size="24px" iconClass="circleDropMenuIcon" />
                <span className="circleDropMenuOp">发送截图</span>
              </div>
          )
        }
      ]}
  ...
  }, [text, props, getTarget, quote, chatType, setThreadMessage, insertChatMessage]);
