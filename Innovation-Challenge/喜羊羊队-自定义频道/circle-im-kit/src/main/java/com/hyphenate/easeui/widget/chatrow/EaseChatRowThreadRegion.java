...
public class EaseChatRowThreadRegion extends FrameLayout implements View.OnClickListener {
    ...
    public void setThreadInfo(EMChatThread info) {
        this.info = info;
        setThreadRegion(info);
        requestLayout();
    }

    private void setThreadRegion(EMChatThread info) {
        if(info == null) {
            setVisibility(GONE);
            return;
        }
        ...
        String num = EaseCommonUtils.handleBigNum(info.getMessageCount());
        tvThreadMsgCount.setText(num);

        EMMessage msgOverview = info.getLastMessage();
        if(msgOverview == null) {
        ...
        Spannable span = EaseSmileUtils.getSmiledText(getContext(), EaseCommonUtils.getMessageDigest(msgOverview, getContext()));
