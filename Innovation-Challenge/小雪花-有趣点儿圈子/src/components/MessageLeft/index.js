...
import RedPacketMsg from "@/components/MyMsg/RedPacketMsg";
...
import TestExt from "@/components/MyMsg/TestExt";
import GameMsg from "@/components/MyMsg/GameMsg";
import DiceGameMsg from "@/components/MyMsg/DiceGameMsg";
import PunchCardMsg from "@/components/MyMsg/PunchCardMsg";
      ...
      <div className={s.message}>
        <div className={s.avatar} onClick={handlerAvatar}>
          <Popover
            ...
            trigger="click"
          >
            <AvatarInfo size={36} src={appUserInfo[message.from]?.avatarurl} extInfo={message.ext} />
          </Popover>
        </div>
        ...
        <div className={s.messageInfo}>
          <div className={s.bar}>
            <div className={s.l}>
              ...
              {message.ext?.robot === 1 && <RobotOutlined /> }

              {message.ext?.rankValue === '1' && <Tag color="red">{message.ext?.rankValue}</Tag> }
              {message.ext?.rankValue === '2' && <Tag color="red">{message.ext?.rankValue}</Tag> }
              {message.ext?.rankValue === '3' && <Tag color="red">{message.ext?.rankValue}</Tag> }
              {message.ext?.rankValue === '4' && <Tag color="red">{message.ext?.rankValue}</Tag> }
              {message.ext?.rankValue === '5' && <Tag color="red">{message.ext?.rankValue}</Tag> }



              {message.ext?.vipValue === '1' && <Tag icon={<VerifiedOutlined />} color="#55acee"/> }
              {message.ext?.vipValue === '2' && <Tag icon={<FacebookOutlined />} color="#55acee"/> }
              {message.ext?.vipValue === '3' && <Tag icon={<LinkedinOutlined />} color="#55acee"/> }
              {message.ext?.vipValue === '4' && <Tag icon={<TwitterOutlined />} color="#55acee"/> }
              {message.ext?.vipValue === '5' && <Tag icon={<YoutubeOutlined />} color="#55acee"/> }
          ...
          <div className={s.content}>
            ...
            {message.type === "custom" && message.customExts?.type === 'invite' && <CustomMsg message={message} />}
            {message.type === "custom" && message.customExts?.type === 'RedPacketMsg' && <RedPacketMsg message={message} />}
            {message.type === "custom" && message.customExts?.type === 'testExt' && <TestExt message={message} />}
            {message.type === "custom" && message.customExts?.type === 'GameMsg' && <GameMsg message={message} />}
            {message.type === "custom" && message.customExts?.type === 'DiceGameMsg' && <DiceGameMsg message={message} />}
            {message.type === "custom" && message.customExts?.type === 'PunchCardMsg' && <PunchCardMsg message={message} />}
